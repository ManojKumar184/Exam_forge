 ...
 */
export function checkMathContext(text: string, startIdx: number, endIdx: number): boolean {
 ...
 */
export function preNormalizeMathText(text: string): string {
  if (!text) return '';
  let out = text;
 ...
 */
export function validateAndFixBraces(latex: string): string {
  let openCount = 0;
  let closeCount = 0;
 ...
 */
export function sanitizeLatexCommands(latex: string): string {
 ...
 */
export function normalizeLatexSyntax(mathText: string): string {
  let text = mathText;
