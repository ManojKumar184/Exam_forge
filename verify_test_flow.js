...
// Define loose models for test database operations
const User = mongoose.models.User || mongoose.model('User', new mongoose.Schema({}, { strict: false }), 'users');
const Subject = mongoose.models.Subject || mongoose.model('Subject', new mongoose.Schema({}, { strict: false }), 'subjects');
const ExamType = mongoose.models.ExamType || mongoose.model('ExamType', new mongoose.Schema({}, { strict: false }), 'examtypes');
const Question = mongoose.models.Question || mongoose.model('Question', new mongoose.Schema({}, { strict: false }), 'questions');
const Paper = mongoose.models.Paper || mongoose.model('Paper', new mongoose.Schema({}, { strict: false }), 'papers');
...
async function seedData() {
  ...
  if (!student) {
    ...
    await student.save();
    console.log('Updated student password to Student@123');
  }

  // Ensure admin is approved
  await User.updateOne({ email: 'admin@examforge.com' }, { approvalStatus: 'approved', role: 'super_admin' });
...
(async () => {
  ...
  try {
    ...
    // ----------------------------------------
    // 1. Try with no code (the required attribute will block form submission, but let's test wrong code first)
    ...
    // 2. Submit with correct access code
    ...
    await page.waitForSelector('.katex, .mjx-container, script[type*="math"]', { timeout: 10000 });
    ...
    const hasOptionMath = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      const mathOptions = buttons.filter(b => b.querySelector('.katex, .mjx-container, script[type*="math"]'));
      return mathOptions.length > 0;
    });
    if (!hasOptionMath) {
    ...
    await page.waitForSelector('.bg-red-50, .border-yellow-200, .bg-amber-50, button', { visible: true });
    ...
    console.log('⏳ Waiting for Result Modal...');
    await page.waitForSelector('.text-amber-500, svg', { timeout: 30000 });
    ...
    const resultStats = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('.bg-white, .rounded-lg, .border'));
    ...
    await page.goto('http://localhost:5173/papers', { waitUntil: 'networkidle0' });
    ...
    await page.goto(paperEditUrl, { waitUntil: 'networkidle0' });
    
    // Wait for the question cards to render
    await page.waitForSelector('.border, .rounded-lg, button', { timeout: 15000 });
